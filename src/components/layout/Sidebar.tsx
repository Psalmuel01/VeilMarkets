    ...
    <>
      ...
      <aside
        className={cn(
          ...
          isMobileOpen ? "translate-x-0" : "-translate-x-full", // Mobile drawer transform
          collapsed ? "lg:w-20" : "w-72", // Width logic
        )}
      >
        {/* Logo */}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={cn(
                    "font-semibold tracking-wide text-sm whitespace-nowrap overflow-hidden transition-all duration-300",
        ...
        {/* Stats Card - Hidden when collapsed ONLY ON DESKTOP */}
        <div
          className={cn(
                  ...
                  <div className="space-y-3 pt-4 border-t border-white/5">
                    <div className="flex justify-between items-center group/bal">
                      <span className="text-[10px] text-muted-foreground group-hover/bal:text-white transition-colors">
                        Aleo Credits (Private)
                      </span>
                      <span className="text-sm font-semibold font-mono text-primary">
                        {balances
                          ? `${balances.private.toLocaleString()}`
                          : "..."}
                      </span>
                    </div>
                    <div className="flex justify-between items-center group/bal">
                      ...
                      <span className="text-sm font-semibold font-mono text-accent">
                        {usdcxBalances
                          ? `${usdcxBalances.private.toLocaleString()}`
                          : "..."}
                      </span>
                    </div>
                    <div className="flex justify-between items-center group/bal">
                      ...
                      <span className="text-sm font-semibold font-mono text-accent">
                        {usadBalances
                          ? `${usadBalances.private.toLocaleString()}`
                          : "..."}
                      </span>
                    </div>
                  </div>
        ...
        <div className="p-3 border-t border-border/50 space-y-2">
          <ConnectWalletButton
            className={
              collapsed ? "w-12 h-12 px-0 justify-center rounded-xl" : "w-full"
            }
            collapsed={collapsed}
          />
        </div>
