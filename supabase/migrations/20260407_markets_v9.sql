...
create table if not exists public.markets_v9 (
  ...
  program_id text not null default 'veilmarkets_core_v9.aleo',
...
-- Ensure `postgres_changes` subscriptions can receive events for this table.
do $$
begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    if not exists (
      ...
      where pubname = 'supabase_realtime'
        ...
        and tablename = 'markets_v9'
    ) then
      execute 'alter publication supabase_realtime add table public.markets_v9';
    end if;
  end if;
end
$$;
